...
%}
    ...
    toe0c = expTable.fl_Contact;
    toe1c = expTable.hl_Contact;
    toe2c = expTable.fr_Contact;
    toe3c = expTable.hr_Contact;
    ...
    for i = 1:size(expTable.Time, 1)
        ...
        % if for i, when contact = 1, 1, when =2, 0, when =nan, do nothing
        if toe0c(i) == 1
            toeFL_contact = 1;
        elseif toe0c(i) == 2
            toeFL_contact = 0;
        ...
        % Spot saved joint data as kn = knee, hx = abductor?, and hy = hip?
        % Spot frame is z->Up, x->Front, y->Left
        if ~isnan(expTable.fl_hy_Load(i))
            if toeFL_contact
                j0 = j0 + 1;
                timeFL(j0, 1) = expTable.Time(i);
                hip_E_FL(j0, 1) = expTable.fl_hy_Load(i);
                kne_E_FL(j0, 1) = expTable.fl_kn_Load(i);
                abd_E_FL(j0, 1) = expTable.fl_hx_Load(i); 
            end

            if toeRL_contact
                j1 = j1 + 1;
                timeRL(j1, 1) = expTable.Time(i);
                hip_E_RL(j1, 1) = expTable.hl_hy_Load(i);
                kne_E_RL(j1, 1) = expTable.hl_kn_Load(i);
                abd_E_RL(j1, 1) = expTable.hl_hx_Load(i);
            end

            if toeFR_contact
                j2 = j2 + 1;
                timeFR(j2, 1) = expTable.Time(i);
                hip_E_FR(j2, 1) = expTable.fr_hy_Load(i);
                kne_E_FR(j2, 1) = expTable.fr_kn_Load(i);
                abd_E_FR(j2, 1) = expTable.fr_hx_Load(i);
            end

            if toeRR_contact
                j3 = j3 + 1;
                timeRR(j3, 1) = expTable.Time(i);
                hip_E_RR(j3, 1) = expTable.hr_hy_Load(i);
                kne_E_RR(j3, 1) = expTable.hr_kn_Load(i);
                abd_E_RR(j3, 1) = expTable.hr_hx_Load(i);
            end
        end
    end
